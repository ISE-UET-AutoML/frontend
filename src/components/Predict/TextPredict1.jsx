	...
	const handleExplain = async (event) => {
		...
		if (selectedIndexes) {
			...
			try {
				...
				console.log(
					'class',
					predictDataState.predictResult[selectedIndexes]
				)
				...
				csvData[selectedIndexes].highlight = data.highlights || []
							...
							{csvData.map((row, index) => (
								...
								>
									...
									{Object.values(row).map((value, i) => (
										<td
											key={i}
											className="px-6 py-2 text-sm font-medium text-gray-900 text-left"
										>
											{value}
										</td>
									))}
									...
									${!falsePredict.includes(index) ? 'bg-emerald-400 hover:text-emerald-400 hover:bg-white hover:border-2 hover:border-emerald-400' : 'bg-red-500 hover:text-red-400 hover:bg-white hover:border-2 hover:border-red-400'}`}
											onClick={() => {
												if (
