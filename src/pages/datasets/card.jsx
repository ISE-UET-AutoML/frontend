...
export default function DatasettCard({ dataset, getDatasets }) {
	const handleDelete = (datasetID) => {
		// if (window.confirm('Are you sure you want to delete this dataset?')) {
		// 	deleteProject(datasetID)
		// 		.then(() => {
				...
				<div className="mt-5 bg-gray-100 group-hover:bg-blue-100 p-6 rounded-xl transition duration-300 relative">
					<div className="flex w-full justify-between items-center">
						<h3 className="text-xl font-medium">
							<a
								href={PATHS.DATASET_VIEW(dataset?._id)}
						...
						{dataset.createdAt && (
							<span>{dayjs(dataset.createdAt).fromNow()}</span>
						)}
					</div>
					<div className="flex w-full justify-between items-center">
