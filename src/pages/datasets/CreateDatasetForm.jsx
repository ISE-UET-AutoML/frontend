...
export default function CreateDatasetForm({
	...
	const [datasetType, setDatasetType] = useState(
		initialValues?.dataset_type || null
	)
	...
	const validateFiles = (files, datasetType) => {
		...
		return files.filter(
			(file) =>
				file?.type && allowedTypes[datasetType]?.includes(file.type)
		)
	}
	...
	const handleFileChange = async (event) => {
		...
		if (['IMAGE'].includes(datasetType)) {
			...
			console.log('Meta files (CSV, XML, JSON):', metaFiles)
		}
		...
		if (datasetType === 'MULTIMODAL' && (!hasImageFolder || !hasCSVFile)) {
			message.error(
				'For MULTIMODAL datasets, upload a folder with images and a CSV file.'
			)
			return
		}
	...
	if (isFolderUpload) {
		...
		const allowedExtensions = {
			...
			AUDIO: '.mp3,.wav,.ogg,.m4a,.flac,.csv,.xml,.json',
			VIDEO: '.mp4,.m4v,.csv,.xml,.json,.mov,.webm,.avi',
		}
