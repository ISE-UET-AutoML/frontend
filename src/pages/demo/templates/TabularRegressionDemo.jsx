...
const TabularRegressionDemo = ({ metadata }) => {
	...
	const buildDisplayLabel = (prediction) => {
		// Handle single {key, value} object (single regression prediction)
		if (
			prediction &&
		...
		// Handle array of {key, value} objects (multiple regression predictions)
		if (
			Array.isArray(prediction) &&
			prediction.length > 0 &&
			'value' in prediction[0]
		) {
	...
	const handleFileUpload = async (event) => {
		...
		try {
			...
			const enhancedPredictions = predictions.map((prediction) => {
				const result = buildDisplayLabel(prediction)
				return result
			})
			...
			setVisibleColumns([...csvColumns, predictedColumnName])
	...
	const handleDownload = () => {
		...
		const csvColumns = Object.keys(finalTable[0]).filter(
			(col) => col !== '_predictionData' && col !== predictedColumnName
		)
	...
	const allColumns = useMemo(() => {
		...
		const allKeys = Object.keys(finalTable[0]).filter(
			(col) => col !== '_predictionData' && col !== 'Confidence'
		)
		return allKeys
	}, [finalTable])
										...
										<tr>
											...
											{finalTable.length > 0 &&
												Object.keys(finalTable[0])
													...
													.map((column) => {
														// Skip predicted column - it'll be added separately
														if (
															column ===
															predictedColumnName
														) {
...
export default TabularRegressionDemo
