...
export default function DeployedModelView() {
    ...
    const handleUploadFiles = async (files) => {
        ...
        try {
            // Make predictions
            const predictRequest = await modelAPI.modelPredict(deployData?.api_base_url, formData)
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            <Space direction="vertical" size="large" className="w-full">
                                        ...
                                        <div className="flex">
                                            <Input.Group compact>
                                                ...
                                                <Button
                                                    type="primary"
                                                    onClick={() => {
                                                        const textToCopy = deployData?.api_base_url || 'https://api.example.com/predict/model-123';
                                                        try {
                ...
                {deployData?.status === 'ONLINE' && !uploading && predictResult && projectInfo && (
                    <>
                        {(() => {
                            if (object) {
                    ...
                    <Row gutter={[16, 16]}>
                        <Col xs={24} sm={12}>
                            <Alert
                                ...
                                type={deployData?.status === 'ONLINE' ? 'warning' : 'info'}
                                showIcon
                                style={{ height: 130 }}
                            />
                            <Button
                                type="default"
                                icon={deployData?.status === 'ONLINE' ? <StopOutlined /> : <CheckCircleOutlined />}
                                size="large"
                                style={{
                                    width: '100%',
                                    fontWeight: 'bold',
                                    marginTop: 15,
                                    backgroundColor: deployData?.status === 'ONLINE' ? '#faad14' : '#2b7fff',
                                    color: 'white',
                                    borderColor: deployData?.status === 'ONLINE' ? '#faad14' : '#2b7fff',
                                }}
                            >
