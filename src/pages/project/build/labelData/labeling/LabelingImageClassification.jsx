...
const LabelingImageClassification = ({
	...
	useEffect(() => {
		setConfig(ImageConfig(currentSavedLabel))
	}, [])

	let tempIndex = 0

	const getTask = (index) => {
		const image = datas[index]
		let annotations = []
	...
	const updateLabelTask = async (image, newLabel) => {
		...
		return await projectAPI.updateAnnotation(projectId, image.id, {
			choice: newLabel,
		})
	}
...
export default LabelingImageClassification
